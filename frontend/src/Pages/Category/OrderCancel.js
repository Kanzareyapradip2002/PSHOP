...
const OrderCancel = () => {
    ...
    const processingItems = allAddToCart.filter(item => item.Confirmation === "Cancel");

    return (
        <div>
            <div className='bg-white p-2'>
                <h2 className='font-bold'>Cancel Order Book</h2>
            </div>
            {allAddToCart.length === 0 ? (
                <p>No items in the cart for this verification code.</p>
            ) : processingItems.length === 0 ? (
                <p>No items are being Cancel .</p>
            ) : (
                    ...
                    {processingItems.map((item, index) => (
                        <Link to={`ConformedPage/${item._id}`}>
                            <li key={index}>
                                ...
                                <div className=' font-bold w-[130px] mt-[-105px] -rotate-45 text-md bg-opacity-75 bg-red-500 rounded-lg'>
                                    <p className='ml-10 pb-1 text-white'>
                                        {item.Confirmation}
                                    </p>
                                </div>
                            </li>
